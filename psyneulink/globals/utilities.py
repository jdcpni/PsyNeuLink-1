...
def np_array_has_single_value(array):
    '''
        Returns
        -------
            True : if **array** is a numpy array, and there is only one value in the array,
                in any dimension
            False : otherwise
    '''
    if not isinstance(array, np.ndarray):
...
def convert_all_elements_to_np_array(arr, cast_from=None, cast_to=None):
    ...
    if isinstance(arr, np.ndarray) and arr.ndim == 0:
        if cast_from is not None and arr.dtype is np.dtype(cast_from):
